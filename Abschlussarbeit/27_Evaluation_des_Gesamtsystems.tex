...
Das Zentrum ist bereits durch den Startmarker mit einer Kantenlänge von 0,120~m belegt, der als einziger Anker der Kartierung eine vergleichsweise kleine Bezugsfläche und damit eine ungenauere Lagebestimmung bietet.
...
Alle Läufe verarbeiten denselben Detektions-Bag und unterscheiden sich ausschließlich in der Parametrierung von \texttt{tagslam.yaml}.
...
Der zweite Versuch übernimmt gemeinsame Einstellungen aus Versuch eins und nutzt zusätzlich das aus der Optimierung in Versuch eins gewonnene Wissen.
...
\begin{figure}[htbp]
    ...
    \caption{Überlagerung der in Versuch~1 erzeugten Karte mit dem Ground Truth}
...
\begin{figure}[htbp]
    ...
    \caption{Überlagerung der in Versuch~2 erzeugten Karte mit dem Ground Truth}
...
\begin{figure}[htbp]
    ...
    \caption{Überlagerung der in Versuch~3 erzeugten Karte mit dem Ground Truth}
...
In $\Delta\psi_z$, dem einzigen weiterhin geschätzten Winkel, liegen beide Läufe bei 0,08~$^\circ$.
...
Die Ursachen sind schlechte Markerdetektionen, sodass der Positionierungsnode keine Daten ausgibt.
...
Der blau hinterlegte Bereich kennzeichnet das Konsistenzintervall von $\pm1\sigma$, das sich aus der Wurzel der Innovationsvarianz $\mathbf{S}$ ergibt.
Diese setzt sich aus der auf den Messraum abgebildeten Zustandskovarianz $\mathbf{P}$ und der Messrauschkovarianz $\mathbf{R}$ zusammen.
...
Dieses Verhalten deckt sich mit dem nachfolgend beschriebenen niedrigen Kalman-Gain und weist auf die konservativ über \texttt{EKF2\_EVP\_NOISE} angesetzte Messrauschkovarianz hin.

An den orange hinterlegten Aussetzern weitet sich das Konsistenzintervall kurzzeitig auf.
Während dieser Phasen schreibt der Zustandsschätzer seinen Zustand allein über das Systemmodell fort, wodurch die Zustandskovarianz $\mathbf{P}$ und mit ihr die Innovationsvarianz $\mathbf{S}$ anwächst.
...
Dies ist keine Eigenschaft der Positionsbestimmung, sondern der Protokollierung, da der Flugcontroller die zugehörigen Topics nur mit dieser Rate auzeichnet.
