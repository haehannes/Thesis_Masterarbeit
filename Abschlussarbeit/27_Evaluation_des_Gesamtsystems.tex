...
Beide werden zunächst hinsichtlich der Durchführung und anschließend hinsichtlich der Evaluation behandelt.
...
Jede dieser Bahnen trägt Markierungen, die eine lagerichtige Ausrichtung der Bögen zueinander sicherstellen.
...
Insgesamt kommen vier verschiedene Markergrößen zum Einsatz: 0,120~m, 0,170~m, 0,300~m und 1,000~m.
Die Größe 0,120~m ist nur ein einziges Mal (\gls{id}~1) vorhanden und dient dem Starten des \gls{uav}s.
Für das Starten des \gls{uav}s sind die anderen Fiducial-Marker-Größen zu groß, um vollständig von der Bodenkamera erfasst zu werden, weswegen eine Lokalisierung nicht möglich ist.
Von der Größe 0,170~m werden 68 Fiducial-Marker geplottet, von der Größe 0,300~m 24 Fiducial-Marker und von der Größe 1,000~m vier Fiducial-Marker.
...
\begin{figure}[htbp]
    ...
    \caption{Tag-Karte der \gls{oic}-Flughalle. Der Kartenursprung (Tag~0) ist grün umrandet}
...
Das Zentrum ist bereits durch den Startmarker mit einer Kantenlänge von 0,120~m belegt, der als einziger Anker der Kartierung eine vergleichsweise kleine Bezugsfläche und damit eine ungenauere Lagebestimmung als Ankerpunkt bietet.
...
Dennoch lässt sich zwischen den Zentren der vier Eckmarker mit einer Kantenlänge von 1,000~m ein Versatz von etwa 5~mm messen.
In Längsrichtung der Bahnen ist der Versatz darauf zurückzuführen, dass sich die Folie nach dem Abziehen der Trägerschicht geringfügig zusammengezogen hat.
In Querrichtung resultiert er aus der großen Bahnlänge und der damit verbundenen erschwerten Handhabung beim Anbringen der Karte.
...
Insgesamt umfasst das aufgezeichnete Material eine Dauer von 166~s, was bei 15~\gls{fps} rund 13,9~\gls{gb} entspricht.


Der erste Bag wird nach Abschnitt~\ref{sec:offline_detektion_apriltags} dem Node \texttt{sync\_and\_detect\_node} übergeben, der die Fiducial-Marker detektiert und die Zeitstempel synchronisiert.
...
Dabei wird der Fall einer unbekannten Karte zugrunde gelegt.
...
Alle Versuche verarbeiten denselben Detektions-Bag und unterscheiden sich ausschließlich in der Parametrierung von \texttt{tagslam.yaml}.
Die Parametrierung erfolgt entsprechend Tabelle~\ref{tab:tagslam_params}.

Die wichtigsten Parameter der jeweiligen Versuche können Tabelle~\ref{tab:tagslam_params_laeufe} entnommen werden.
...
\texttt{minimum\_viewing\_angle}                & $15{,}0$ & $15{,}0$ & $15{,}0$ \\
\texttt{minimum\_tag\_area}                     & $2500$ & $2500$ & $2500$ \\
\texttt{use\_approximate\_sync}                 & \texttt{true} & \texttt{true} & \texttt{true} \\
\bottomrule
\end{tabular}
...
Es wird außer dem Ursprungsmarker keine weitere Fixierung vorgenommen, sodass die restlichen Fiducial-Marker relativ zu Fiducial-Marker 0 optimiert werden.
...
Der zweite Versuch übernimmt gemeinsame Einstellungen aus Versuch eins und nutzt zusätzlich das aus der Optimierung gewonnene Wissen.
Um fehlerhafte Messungen von der Karte fernzuhalten, wird die Schwelle \texttt{max\_subgraph\_error} auf 10 herabgesetzt.
Zugleich sinkt \texttt{max\_num\_incremen\-tal\_opt} auf 10, wodurch der Faktorgraph häufiger vollständig optimiert wird und die Lösung an Genauigkeit gewinnt.
...
Der dritte Versuch behält die Einstellungen des zweiten Versuchs weitgehend bei, es wurde jedoch der Betriebsmodus \texttt{slow} gewählt, um die Genauigkeit weiter zu steigern.
...
Aufgrund der positiven Ergebnisse und der hohen Laufzeit im Betriebsmodus \texttt{slow} wurde auf einen Vergleichslauf mit \texttt{full} verzichtet.

\subsection{Evaluation der Kartierung}
\label{sec:evaluation_kartierung}
Der Ground Truth dient als Grundlage für die Bewertung der Kartierungsqualität.
...
Der Fiducial-Marker mit der \gls{id}~202 fehlt in allen Fällen.
Es handelt sich um einen der Eckmarker, für dessen vollständige Abbildung im Bild ein entsprechend großer Abstand zum Boden erforderlich ist.
Dieser Fiducial-Marker wurde nicht sorgfältig genug bei der Aufnahme des Bags betrachtet.

Nachfolgende Abbildungen zeigen die aufgenommenen Karten aus der Draufsicht der drei Versuche und vergleichen diese mit dem Ground Truth.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{Bilder/karten_vergleich_ueberlagert_versuch_1.png}
    \caption{Überlagerung der in Versuch~eins erzeugten Karte mit dem Ground Truth}
...
\begin{figure}[htbp]
    ...
    \caption{Überlagerung der in Versuch~zwei erzeugten Karte mit dem Ground Truth}
...
\begin{figure}[htbp]
    ...
    \caption{Überlagerung der in Versuch drei erzeugten Karte mit dem Ground Truth}
...
Erreicht wird dies bei einer Laufzeit von etwa 45~Minuten und ohne jedes Vorwissen über die Kartengeometrie.
...
Gleiches gilt für die Verkippungen von 0,33~$^\circ$ in $\Delta\varphi_x$ und 0,22~$^\circ$ in $\Delta\theta_y$.
...
In $\Delta\psi_z$, dem einzigen weiterhin geschätzten Winkel, liegen beide Versuche bei 0,08~$^\circ$.
...
Jeder Punkt entspricht damit einem Fiducial-Marker in einem der drei Versuche.
...
In der Nähe des Ursprungs liegen die Abweichungen aller drei Versuche bei wenigen Millimetern.
...
Zur Verdeutlichung dieses Zusammenhangs ist je Versuch eine Ausgleichsgerade durch den Ursprung eingezeichnet.
Ihre Steigung beschreibt den Zuwachs der Abweichung je Meter Abstand zum Ursprung.
Für den ersten Versuch beträgt sie 9,93~mm/m, für den zweiten Versuch 7,66~mm/m und für den dritten Versuch 7,39~mm/m.
Der zweite und der dritte Versuch liegen damit sehr nahe beieinander, während der erste Versuch über den gesamten Abstandsbereich die größten Abweichungen aufweist.
...
Für die Kartierung einer neuen oder die Erweiterung einer bestehenden Umgebung empfiehlt sich das zweistufige Vorgehen aus Versuch eins und Versuch zwei.
Die Parametrierung des ersten Versuchs liefert innerhalb von etwa 45~Minuten eine Startschätzung, ohne dass Vorwissen über die Kartengeometrie erforderlich ist.
...
Im letzten Teil werden die Ergebnisse der Testflüge bewertet.
...
Die Flugführung übernimmt der eigens entwickelte Node \texttt{control\_console\_uxrce}, der auf dem Begleitrechner ausgeführt wird.
...
Ist das System bereits armiert, löst dieselbe Taste eine Landung an Ort und Stelle aus. 
Dabei bleiben die horizontale Position und der Gierwinkel konstant, 
während die Höhe langsam abgesenkt wird, bis das System bei Bodenkontakt automatisch disarmiert wird.
...
Seitenlänge sowie Geschwindigkeit können über die Startparameter des Nodes eingestellt werden.
...
Ein Multicopter giert, indem er das Reaktionsmoment der Rotoren gezielt verstimmt, wozu ein Teil der Motoren beschleunigt und der andere abgebremst wird.
...
Ein schneller Gierbefehl bringt die Rotoren an ihre Leistungsgrenze, sodass sie das für Giermoment und Auftrieb gleichzeitig erforderliche Drehmoment nicht mehr aufbringen können.
...
Der zweite stoppt als Notausschalter die Rotoren unmittelbar und dient als letzte Sicherheitsmaßnahme. 
...
\texttt{isaac\_reference\_size}            & 0,170~m \\
...
Die Ausgaberate des Nodes liegt mit \texttt{publish\_rate\_hz}~=~50~Hz bewusst darunter.
Damit übertrifft die Ausgaberate die in Abschnitt~\ref{sec:pos_ekf_konfiguration} geforderte untere Grenze von 30~\gls{fps} deutlich.
Die Reserve zur Bildrate sichert den Takt selbst dann, wenn einzelne Bilder verzögert eintreffen oder ausfallen.
...
Der so berechnete Schwellenwert wird durch zwei feste Grenzen beschränkt.
...
Über \texttt{ransac\_min\_inliers\_ratio}~=~0,70 wird nach Gleichung~\ref{eq:pos_ransac_konsens} gefordert, dass mindestens 70~\% aller Schätzungen der Konsensmenge angehören.

Die Gewichtung nach der Kartengüte benötigt eine Referenzgröße, gegen die die kartierte Unsicherheit jedes Markers verglichen wird.
Der Parameter \texttt{map\_cov\_reference\_sigma\_m}~=~0,02~m setzt die Referenz-Standardabweich\-ung $\sigma_{\mathrm{ref}}$ aus Gleichung~\ref{eq:pos_map_quality}.
...
Im Vorhinein wurden Testflüge mit dem Ground Truth und den Versuchskarten eins bis drei durchgeführt, ohne einen merklichen Unterschied in der Positioniergenauigkeit festzustellen.
Bei den aufgezeichneten Karten wurde jeweils der Fiducial-Marker mit der \gls{id}~202 händisch nachgetragen.
Die eigentliche Versuchsdurchführung fand mit dem Ground Truth statt. 
...
Anschließend wird über die Taste \texttt{g} der Demo-Modus aktiviert, in dem das \gls{uav} ein Quadrat mit einer Seitenlänge von 2,0~m abfliegt und danach an der Startecke schwebt.
...
Da sich die Flüge im Ergebnis kaum unterscheiden, erfolgt die Auswertung beispielhaft an einem von ihnen.
...
Der Steigflug führt auf die Sollhöhe von 1,8~m, der Demo-Modus beginnt bei 23,0~s und das \gls{uav} fliegt ein Quadrat mit einer Seitenlänge von 2,0~m, bevor der Sinkflug bei 64,6~s einsetzt.
Über 97,1~\% der Zeit steht das \gls{uav} im Modus \texttt{Offboard}, den es erst zur Landung verlässt.
...
Bewertet wird stattdessen die Konsistenz der Fusion, also die Frage, ob die eingespeiste Position und die aus der \gls{imu} fortgeschriebene Vorhersage zueinander passen.
...
\caption{Zeitliche Belegung der Stützquellen des \gls{ekf}s eines Testflugs}
...
Die Ursache für das Fehlen der Positionswerte kann aus den Protokolldateien nicht direkt abgeleitet werden.
Während der Aussätzer wurden von dem Node zur Positionsbestimmung keine Daten gesendet,
sodass als Ursache schlechte Markerdetetkionen nahe liegen.
...
\newpage
\subsection*{Innovation der Vision-Stützung}
\label{subsec:innovation}
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{Bilder/06_ev_innovationen.png}
\caption{Innovationen der Vision-Stützung mit den zugehörigen Konsistenzgrenzen}
\label{fig:ev_innovationen}
\end{figure}
\FloatBarrier

Abbildung~\ref{fig:ev_innovationen} zeigt die Innovation der Vision-Stützung für die vier Messgrößen Position~$x$, Position~$y$, Höhe und Gierwinkel.
Die Innovation bezeichnet die Differenz zwischen der eingespeisten Vision-Messung und dem aus dem Systemmodell vorhergesagten Messwert.
Sie ist damit das unmittelbare Maß für die Konsistenz der Fusion. 

Der blau hinterlegte Bereich kennzeichnet das Konsistenzintervall von $\pm1\sigma$, das sich aus der Wurzel der Innovationsvarianz \gls{sym:S} ergibt.
Diese setzt sich aus der auf den Messraum abgebildeten Zustandskovarianz \gls{sym:P} und der Messrauschkovarianz \gls{sym:Rmeas} zusammen.
Ein konsistenter Filter hält seine Innovation innerhalb dieses Intervalls.

Über den gesamten Flug bleibt die Innovation aller vier Messgrößen innerhalb des Konsistenzintervalls.
Der Filter divergiert zu keinem Zeitpunkt und übernimmt die eingespeiste Pose konsistent.
Zugleich schöpft die Innovation das Band nur zu einem Bruchteil aus und streut deutlich enger, als es die angesetzte Unsicherheit erwarten ließe.
Dieses Verhalten deckt sich mit dem nachfolgend beschriebenen niedrigen Kalman-Gain und weist auf eine konservativ über \texttt{EKF2\_EVP\_NOISE} angesetzte Messrauschkovarianz hin.

An den orange hinterlegten Aussetzern weitet sich das Konsistenzintervall kurzzeitig auf.
Während dieser Phasen schreibt der Zustandsschätzer seinen Zustand allein über das Systemmodell fort, wodurch die Zustandskovarianz \gls{sym:P} und mit ihr die Innovationsvarianz \gls{sym:S} anwächst.
Die Aufweitung fällt damit mit den Spitzen des Kalman-Gains zusammen.
...
Der Filter befindet sich dort im eingeschwungenen Zustand, in dem jedes Update genau so viel Unsicherheit abbaut, wie seit der letzten Korrektur durch das Prozessrauschen \gls{sym:Q} hinzugekommen ist.
Die Zustandskovarianz \gls{sym:P} bleibt dadurch klein und stabil.
...
Die Zustandskovarianz wächst dabei ausschließlich durch das Prozessrauschen an, während die Messrauschkovarianz \gls{sym:Rmeas} unverändert bleibt.
...
Der Parameter \texttt{EKF2\_EVP\_NOISE} gibt für die Vision-Position eine Standardabweichung von 0,1~m vor, die der Filter wegen \texttt{EKF2\_EV\_NOISE\_MD}~=~1 für jede Messung ansetzt.
...
Für weiterführende Arbeiten bietet sich an, die tatsächliche Streuung der Vision-Lokalisierung zu bestimmen und die Annahme für \gls{sym:Rmeas} daran auszurichten.
Fällt die tatsächliche Streuung geringer aus als die angenommenen 0,1~m, kann der Parameter abgesenkt oder über \texttt{EKF2\_EV\_NOISE\_MD}~=~0 durch eine pro Nachricht berechnete Kovarianz ersetzt werden.
Die Vision-Messungen würden dann stärker gewichtet werden und die Positionsunsicherheit des Filters weiter sinken.
...
Dies ist keine Eigenschaft der Positionsbestimmung, sondern der Protokollierung, da der Flugcontroller die zugehörigen Topics nur mit dieser Rate aufzeichnet.
...
Ihre Verteilung folgt keinem Muster der Flugdynamik, denn die Lagewinkel bleiben über den gesamten Flug unterhalb von 3,2~$^\circ$ und unterscheiden sich vor den Aussetzern nicht von den Werten des ungestörten Reiseflugs.
Stattdessen sind die Aussetzer räumlich konzentriert.
Bei einer $y$-Position unterhalb von null rechnet der Zustandsschätzer je nach Bereich zwischen 4,6 und 16,9~\% der Zeit im \gls{drl}-Modus, während oberhalb von null trotz eines Aufenthalts von knapp 16~s kein einziger Aussetzer auftritt.
...
Zunächst wird die Kartierung und anschließend die Positionierung betrachtet.
...
Als tragfähig erweist sich das zweistufige Vorgehen aus Versuch eins und Versuch zwei.
Der erste Versuch liefert ohne jedes Vorwissen über die Kartengeometrie innerhalb von etwa 45~Minuten eine grobe erste Schätzung, der zweite verfeinert diese unter Einbringen von Wissen über die Karte innerhalb von etwa zwei Stunden.
Der Betriebsmodus \texttt{slow} aus Versuch drei rechtfertigt den vierfachen Rechenaufwand nicht, da er den mittleren räumlichen Abstand um lediglich 0,41~mm senkt und den Maximalwert sogar leicht anhebt.
Die Güte der Karte wird damit weniger durch den Betriebsmodus als durch das eingebrachte Vorwissen bestimmt.
...
Da alle Fiducial-Marker allein anhand des Ursprungsmarkers optimieren werden, baut sich der Fehler über die gesamte Kartenausdehnung auf und begrenzt die Skalierbarkeit des Verfahrens.
...
Als Ground Truth dient die digitale Konstruktion der Druckvorlage und nicht eine unabhängige Vermessung der angebrachten Karte.
...
Auch die Positionierung ist vollständig aus quelloffener und frei verfügbarer Software aufgebaut, deren Verarbeitungsschritte über \gls{ros2} als Softwaregrundlage miteinander verbunden sind.
...
Eine Abweichung im Montagewinkel der Kamera wirkt sich direkt auf den Nickwinkel aus, der zur Positionsbestimmung des Zustandsschätzers nicht herangezogen wird.
...
Der Parameter \texttt{EKF2\_EVP\_NOISE} gibt eine Standardabweichung von 0,1~m vor, die der Zustandsschätzer für jede Messung unverändert ansetzt.
...
Da sich die Pose des \gls{uav}s auf diese Markerlagen stützt, überträgt sich der Kartenfehler auf die eingespeiste Position.
...
Über die Fernsteuerung kann jederzeit in den Flug eingegriffen, in den Modus \texttt{Auto.Land} gewechselt oder die Rotoren über den Notausschalter abgeschaltet werden.
